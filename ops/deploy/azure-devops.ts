...
export async function getExtensionInfo(serverUrl: string, azureDevOpsToken: string, publisherName: string, extensionId: string): Promise<ExtensionManagementInterfaces.InstalledExtension> {
    ...
    return extensionManagementApiObject.getInstalledExtensionByName(publisherName, extensionId);
}

export async function uninstallExtension(serverUrl: string, azureDevOpsToken: string, publisherName: string, extensionId: string): Promise<void> {
    const webApi: nodeApi.WebApi = await getWebApi(serverUrl, azureDevOpsToken);
    const extensionManagementApiObject: ExtensionManagementApi.IExtensionManagementApi = await webApi.getExtensionManagementApi();
    // console.log('States:', await extensionManagementApiObject.getStates(true, true, true));

    // Although this API claims to be "ByName", it actually corresponds to the the `extensionId`. The same weirdness exists when you use the az devops CLI
    return extensionManagementApiObject.uninstallExtensionByName(publisherName, extensionId);
}

export async function installExtension(serverUrl: string, azureDevOpsToken: string, publisherName: string, extensionId: string): Promise<ExtensionManagementInterfaces.InstalledExtension>{
  const webApi: nodeApi.WebApi = await getWebApi(serverUrl, azureDevOpsToken);
  const extensionManagementApiObject: ExtensionManagementApi.IExtensionManagementApi = await webApi.getExtensionManagementApi();
  // console.log('States:', await extensionManagementApiObject.getStates(true, true, true));

  // Although this API claims to be "ByName", it actually corresponds to the the `extensionId`. The same weirdness exists when you use the az devops CLI
  return extensionManagementApiObject.installExtensionByName(publisherName, extensionId);
}
