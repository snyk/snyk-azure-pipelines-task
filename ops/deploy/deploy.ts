...
function checkVersionsMatch(): boolean {
    ...
    const packageJsonVersion: string = JSON.parse(fs.readFileSync(packageJsonFilePath, "utf8")).version;
    const versionFromExtensionFile: string = JSON.parse(fs.readFileSync(extensionFilePath, "utf8")).version;
    ...
    return (packageJsonVersion === versionFromExtensionFile &&
...
export async function publishExtension(target: DeployTarget, workingDirectory: string, taskRelativePath: string, publishArgs: ExtensionPublishArgs, newVersion?: string): Promise<void> {
...
export async function getExtensionInfo(publishArgs: ExtensionPublishArgs): Promise<ExtensionManagementInterfaces.InstalledExtension> {
    const azUrl = getAzUrl(publishArgs.azureOrg);
    return myAz.getExtensionInfo(azUrl, publishArgs.azureDevopsPAT, publishArgs.vsMarketplacePublisher, publishArgs.extensionId);
}

export async function uninstallExtension(publishArgs: ExtensionPublishArgs): Promise<void> {
    const azUrl = getAzUrl(publishArgs.azureOrg);
    return myAz.uninstallExtension(azUrl, publishArgs.azureDevopsPAT, publishArgs.vsMarketplacePublisher, publishArgs.extensionId);
}

export async function installExtension(publishArgs: ExtensionPublishArgs): Promise<ExtensionManagementInterfaces.InstalledExtension> {
    const azUrl = getAzUrl(publishArgs.azureOrg);
    return myAz.installExtension(azUrl, publishArgs.azureDevopsPAT, publishArgs.vsMarketplacePublisher, publishArgs.extensionId);
}
...
function getExtensionPublishArgsFromConfigFile(configFilePath: string): ExtensionPublishArgs {
...
export async function mainWithRawArgs(inputArgs: string[], workingDirectory: string, taskRelativePath: string) {
    ...
    if (parsedArgs.command === Command.Deploy) {
        ...
        if (parsedArgs.target === DeployTarget.Dev ||
            parsedArgs.target === DeployTarget.Prod) {
            publishArgs = getExtensionPublishArgsFromEnvVars();
        } else {
            publishArgs = getExtensionPublishArgsFromConfigFile(parsedArgs.configFile);
        }

        await updateOrInstallExtension(parsedArgs, workingDirectory, taskRelativePath, publishArgs);
    }
...
export async function updateOrInstallExtension(parsedArgs: InputArgs, workingDirectory: string, taskRelativePath: string, publishArgs: ExtensionPublishArgs): Promise<void> {
    ...
    await publishExtension(parsedArgs.target, workingDirectory, taskRelativePath, publishArgs, parsedArgs.newVersion);
